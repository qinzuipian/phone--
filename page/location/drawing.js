...
(function () {
    ...
    (function () {
        ...
        b.lang.Class.prototype.addEventListener = function (j, i, h) {
            if (!b.lang.isFunction(i)) { return } !this.__listeners && (this.__listeners = {});
            var g = this.__listeners, k;
            if (typeof h == "string" && h) {
                if (/[^\w\-]/.test(h)) { throw ("nonstandard key:" + h) }
                else { i.hashCode = h; k = h }
            } j.indexOf("on") != 0 && (j = "on" + j);
            typeof g[j] != "object" && (g[j] = {});
            k = k || b.lang.guid();
            i.hashCode = k; g[j][k] = i
        };
        b.lang.Class.prototype.removeEventListener = function (i, h) {
            if (b.lang.isFunction(h)) { h = h.hashCode }
            else { if (!b.lang.isString(h)) { return } } !this.__listeners && (this.__listeners = {});
        ...
        b.lang.Class.prototype.dispatchEvent = function (k, g) {
            if (b.lang.isString(k)) { k = new b.lang.Event(k) } !this.__listeners && (this.__listeners = {});
            g = g || {};
            for (var j in g) { k[j] = g[j] } var j,
                h = this.__listeners,
                l = k.type;
        ...
        b.insertHTML = b.dom.insertHTML = function (j, g, i) {
            j = b.dom.g(j);
            var h, k;
            if (j.insertAdjacentHTML) { j.insertAdjacentHTML(g, i) }
            else {
        ...
        b.on = b.event.on = function (h, k, m) {
            k = k.replace(/^on/i, "");
            h = b._g(h);
            var l = function (o) { m.call(h, o) },
                g = b.event._listeners,
                j = b.event._eventFilter, n, i = k;
                k = k.toLowerCase();
            if (j && j[k]) {
                n = j[k](h, k, l);
            ...
            if (h.attachEvent) 
             { h.attachEvent("on" + i, l) } } g[g.length] = [h, k, m, l, i]; 
             return h
        };
        b.un = b.event.un = function (i, l, h) {
            ...
            while (j--) {
                ...
                if (n[1] === l && n[0] === i && (k || n[2] === h)) {
                    m = n[4]; g = n[3];
                    if (i.removeEventListener) { i.removeEventListener(m, g, false) }
                    else { if (i.detachEvent) { i.detachEvent("on" + m, g) } } o.splice(j, 1)
                }
            }
            return i
        };
    ...
    d.prototype._initialize = function (h, g) {
        ...
        if (g.enableDrawingTool) {
            var i = new a(this,
                g.drawingToolOptions);
            this._drawingTool = i;
            h.addControl(i)
        }
        if (g.enableCalculate === true) { this.enableCalculate() }
        else { this.disableCalculate() }
    ...
    d.prototype._close = function () {
        this._isOpen = false;
        if (this._mask) { this._map.removeOverlay(this._mask) }
        if (this._drawingTool) { this._drawingTool.setStyleByDrawingMode("hander") }
    };
    d.prototype._bindMarker = function () {
        ...
        h.addEventListener("touchstart", g)
    };
    d.prototype._bindCircle = function () {
        var m = this,
            ...
            k = null;
        var j = function (p) {
            if (m.controlButton == "right" && (p.button == 1 || p.button == 0)) { return } k = p.point;
            i = new BMap.Circle(k, 0, m.circleOptions);
            h.addOverlay(i);
            o.enableEdgeMove();
            o.addEventListener("touchmove", n);
            b.on(document, "touchend", l)
        };
        var n = function (p) {
            i.setRadius(m._map.getDistance(k, p.point));
            // h.disableDragging();  //禁止拖拽
        };
        var l = function (q) {
            ...
            b.un(document, "touchend", l)
        };
        var g = function (p) {
            b.preventDefault(p);
            b.stopBubble(p);
            if (m.controlButton == "right" && p.button == 1) { return }
            if (k == null) { j(p) }
        };
    ...
    d.prototype._bindPolylineOrPolygon = function () {
        ...
        overlay = null,
            isBinded = false;
        var l = function (o) {
            if (k.controlButton == "right" && (o.button == 1 || o.button == 0)) { return } j.push(o.point);
            ...
            if (j.length == 1) {
                ...
                if (k._drawingType == BMAP_DRAWING_POLYGON) { overlay = new BMap.Polygon(n, k.polygonOptions) }
                } m.addOverlay(overlay)
                }                         
            else { overlay.setPath(n) }
            if (!isBinded) {
                isBinded = true;
                h.enableEdgeMove();
                h.addEventListener("touchmove", i);
                h.addEventListener("touchend", g)
            }
        };
        var i = function (o) {
        ...
        var g = function (p) {
            ...
            isBinded = false;
            h.disableEdgeMove();
            h.removeEventListener("touchstart", l);
            h.removeEventListener("touchmove", i);
            h.removeEventListener("touchend", g);
    ...
    d.prototype._bindRectangle = function () {
        ...
        var m = function (p) {
            ...
            j = p.point;
            var o = j;
            i = new BMap.Polygon(k._getRectanglePoint(j, o),
            k.rectangleOptions);
            n.addOverlay(i);
            h.enableEdgeMove();
            h.addEventListener("touchmove", l);
            b.on(document, "touchend", g)
        };
        var l = function (o) {
        ...
        var g = function (p) {
            var o = k._calculate(i,
            i.getPath()[2]);
            k._dispatchOverlayComplete(i, o);
            j = null;
            h.disableEdgeMove();
            h.removeEventListener("touchmove", l);
            b.un(document, "touchend", g)
        };
        h.addEventListener("touchstart", m)
    };
    d.prototype._calculate = function (j, i) {
        ...
        if (this._enableCalculate && BMapLib.GeoUtils) {
            var k = j.toString(); 
            switch (k) {
                case "[object Polyline]": h.data = BMapLib.GeoUtils.getPolylineDistance(j);
                    break;
                case "[object Polygon]": h.data = BMapLib.GeoUtils.getPolygonArea(j);
                    break;
                case "[object Circle]": var g = j.getRadius();
                    h.data = Math.PI * g * g;
                    break
            }
            if (!h.data || h.data < 0) { h.data = 0 }
            else { h.data = h.data.toFixed(2) } h.label = this._addLabel(i, h.data)
         } 
         return h
    };
    ...
    function e() { this._enableEdgeMove = false }
    ...
    e.prototype.initialize = function (i) {
        ...
        this._map.addEventListener("resize",
            function (k) { h._adjustSize(k.size) });
        this._map.getPanes().floatPane.appendChild(j);
        this._bind();
        return j
    };
    e.prototype.draw = function () {
        var i = this._map, g = i.pixelToPoint(new BMap.Pixel(0, 0)),
    ...
    e.prototype._bind = function () {
        ...
        var o = [
            ...
            i = o.length;
        while (i--) { b.on(h, o[i], j) } b.on(h,
            "touchmove",
            function (p) {
                if (l._enableEdgeMove) { l.mousemoveAction(p) }
            })
    };
    ...
    e.prototype.getDrawPoint = function (l) {
        var k = this._map,
        j = b.getTarget(l),
        h = l.offsetX || l.layerX || 0, m = l.offsetY || l.layerY || 0;
    ...
    g = this.drawingToolOptions = g || {};
        ...
        if (g.drawingModes) { this.drawingModes = g.drawingModes }
        else { this.drawingModes = this.defaultDrawingModes }
        if (g.anchor) { this.setAnchor(g.anchor) }
        if (g.offset) { this.setOffset(g.offset) }
    }
    ...
    a.prototype._generalHtml = function (m) {
        ...
        for (var j = 0,
            g = this.drawingModes.length;
            j < g;
             j++) {
             var l = "BMapLib_box BMapLib_" + this.drawingModes[j];
            if (j == g - 1) { l += " BMapLib_last" }
             k.push(n(l,
            this.drawingModes[j]))
        }
        return k.join("")
    ...
    a.prototype._setScale = function (j) {
        ...
        "margin-left:0px\\0;",
        "margin-top:0px\\0;",
        "filter: progid:DXImageTransform.Microsoft.Matrix(", "M11=" + j + ",", "M12=0,", "M21=0,", "M22=" + j + ",",
    ...
    a.prototype.setStyleByDrawingMode = function (h) {
        ...
        for (var k = 0, g = j.length; k < g; k++) {
            ...
            if (m.getAttribute("drawingType") == h) {
                ...
                if (k == g - 1) { l += " BMapLib_last" } m.className = l
            }
            else { m.className = m.className.replace(/_hover/, "") }
        }
    };
